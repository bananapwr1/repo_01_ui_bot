# 🤖 UI Bot - Торговый Telegram-бот

UI-бот для взаимодействия пользователей с торговой системой через Telegram.

## 🎯 Основные функции

- **Telegram интерфейс** - пользователи взаимодействуют с системой через Telegram
- **API-сервер** - предоставляет REST API для связи с Ядром Анализа (Render)
- **Шифрование данных** - безопасное хранение учетных данных Pocket Option
- **Интеграция с Supabase (опционально)** - внешние таблицы для сигналов и запросов (без хранения профилей пользователей)

## 📋 Компоненты

### 1. Telegram Bot
- `/start` - приветствие и информация
- `/set_po [логин] [пароль]` - сохранение зашифрованных учетных данных
- `/signal` - запрос торгового сигнала

### 2. FastAPI сервер
- `POST /get_po_credentials` - эндпоинт для получения учетных данных пользователя

### 3. Локальная БД (SQLite)
- Хранение зашифрованных учетных данных пользователей
- Быстрый доступ к данным без обращения к внешним сервисам

## 🛠️ Технологии

- **Python 3.12+**
- **python-telegram-bot** - Telegram Bot API
- **FastAPI + Uvicorn** - REST API сервер
- **Supabase** - облачная база данных
- **Cryptography (Fernet)** - шифрование данных
- **SQLite** - локальное хранение данных
- **python-dotenv** - управление переменными окружения

## 🚀 Быстрый старт

### 1. Установка зависимостей

```bash
pip install -r requirements.txt
```

### 2. Настройка переменных окружения

Создайте файл `.env` (или настройте переменные на Bothost):

```bash
# Telegram token (canonical)
BOT_TOKEN=your_telegram_bot_token

ENCRYPTION_KEY=your_fernet_encryption_key
PORT=8000

# Опционально (только если нужен функционал сигналов через внешнюю БД):
SUPABASE_URL=https://your-project.supabase.co
SUPABASE_KEY=your_supabase_public_key

# Опционально (root admin-команды):
ADMIN_USER_ID=123456789
```

### 3. Запуск

```bash
python main.py
```

## 📁 Структура файлов

```
.
├── main.py                 # Главный файл (Telegram Bot + API)
├── user_db_handler.py      # Управление локальной БД
├── crypto_utils.py         # Шифрование/расшифрование
├── requirements.txt        # Зависимости Python
├── .env                    # Переменные окружения (не в git!)
├── .gitignore             # Игнорируемые файлы
├── README.md              # Этот файл
└── BOTHOST_SETUP.md       # Инструкции по деплою на Bothost
```

## 🔒 Безопасность

- ✅ Все учетные данные шифруются с помощью Fernet (AES-128)
- ✅ Переменные окружения не хранятся в коде
- ✅ `.env` файл исключен из git
- ✅ Локальная БД содержит только зашифрованные данные
- ⚠️ **Важно**: Используйте надежные ключи шифрования и храните их в безопасности

## 📡 API Endpoints

### POST /get_po_credentials

Получение зашифрованных учетных данных пользователя для Pocket Option.

**Request:**
```json
{
  "user_id": 123456789,
  "request_source": "trading_core"
}
```

**Response (Success):**
```json
{
  "status": "success",
  "user_id": 123456789,
  "login_enc": "gAAAAABh...",
  "password_enc": "gAAAAABh..."
}
```

**Response (Error):**
```json
{
  "detail": "Credentials not found for user 123456789"
}
```

## 🔧 Разработка

### Локальное тестирование

```bash
# Проверка синтаксиса
python -m py_compile main.py user_db_handler.py crypto_utils.py

# Проверка переменных окружения
python -c "from dotenv import load_dotenv; import os; load_dotenv(); print('OK' if all([os.getenv('BOT_TOKEN'), os.getenv('ENCRYPTION_KEY')]) else 'MISSING VARS')"
```

### Тестирование шифрования

```python
from crypto_utils import encrypt_data, decrypt_data

test = "password123"
encrypted = encrypt_data(test)
decrypted = decrypt_data(encrypted)

assert test == decrypted, "Encryption/Decryption failed!"
print("✅ Encryption works!")
```

## 📦 Деплой

Подробные инструкции по деплою на Bothost см. в файле [BOTHOST_SETUP.md](./BOTHOST_SETUP.md).

### Основные шаги:
1. Подключите репозиторий к Bothost
2. Установите все переменные окружения
3. Запустите бот
4. Проверьте логи
5. Получите внешний URL бота

## 🐛 Отладка

### Частые проблемы:

**1. `supabase_key is required`**
- Проверьте переменные `SUPABASE_URL` и `SUPABASE_KEY`
- Убедитесь, что они установлены в окружении

**2. `ENCRYPTION_KEY is required`**
- Установите переменную `ENCRYPTION_KEY` в `.env` или на хостинге
- Ключ должен быть валидным Fernet-ключом (44 символа base64)

**3. `Module not found`**
- Установите зависимости: `pip install -r requirements.txt`

## 🔗 Связь с другими компонентами

- **Admin Bot** - управление системой и мониторинг
- **Ядро Анализа (Render)** - обработка торговых сигналов и анализ рынка
- **Supabase** - централизованное хранилище данных
- **Pocket Option API** - выполнение торговых операций

## 📝 Логи

Бот выводит логи в формате:

```
INFO:__main__:🚀 Запуск UI-Бота (Telegram + API Server)...
INFO:uvicorn:Started server process
INFO:uvicorn:Waiting for application startup.
INFO:uvicorn:Application startup complete.
INFO:uvicorn:Uvicorn running on http://0.0.0.0:8000
```

## 🤝 Поддержка

При возникновении проблем:
1. Проверьте логи бота
2. Убедитесь, что все переменные окружения установлены
3. Проверьте файл [BOTHOST_SETUP.md](./BOTHOST_SETUP.md) для решения типичных проблем

---

**Статус**: ✅ Готово к деплою  
**Последнее обновление**: 2024-12-11  
**Версия**: 1.0.0
